...
// keeps input disabled when timer is not running//
document.getElementById("input").disabled = true; 


// blanker function performs different function and is initialized when start button is clicked//
function blanker() {
...
//randomizer function creates 100 span element inside display class each having span number as additional class name//
function randomizer() {
for (let i=0; i<100; i++) {
    idnumber = "span" + i; 
...
//makes a spannumber class inside display to have a class with name active and removes the previous active span//
function activeSpan() {
    if (starter>0) {
        destarter = starter - 1;
...
//inputSpanCreater function creates 100 inputspan elements inside input element same way as randomizer does inside display//
...
//cleans the input value once blankspace is clicked ie one inputspan is filled//
...
//timeRed() controls the duration of test//
remainingTime = 59;
function timerRed() {
    remainingTime--;
...
//compares the texts inside the span element of display and inputspan element of input and decides if input value is correct. styles the color red incase it is wrong and styles the color of span element green if it is typed correctly//
function comparer() {
...
// cleans the element with id name display to show the result//
function disBlanlk() {
    document.getElementById("display").innerHTML = "";
}

// adds the class name and shows the result//
function resultShow() {  
